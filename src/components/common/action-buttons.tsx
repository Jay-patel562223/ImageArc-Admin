...
import StateForm from '../country/state-form'
...
const ActionButtons = ({
  ...
  function handleProductStatus(type: string) {
    openModal('PRODUCT_STATUS',  {id, type} );
  }
  ...
  return (
    <div className="gap-8 inline-flex w-auto items-center">
      ...
      {approveButton &&
        ...
        {isStateAdd && (
            <button
            onClick={handleStateRedirect}
            className="text-accent transition duration-200 hover:text-accent-hover focus:outline-none"
          >
            {t('table:table-item-add-state')}
          </button>
          )}
      {userStatus && (
        <>
      ...
      {approveProduct && (
        <> {isApproved  ?  (
          <button
            ...
            className="text-green-500 transition duration-200 hover:text-red-600 focus:outline-none"
            title={t('common:text-product-disapprove')}
          >
           <CheckMarkCircle width={20} />
          </button>
        ) :  (
          <button
            onClick={() => handleProductStatus('pending')}
            className="text-red-500 transition duration-200 hover:text-red-600 focus:outline-none"
            title={t('common:text-product-approve')}
          >
            <BanUser width={20} />
          </button>
        )}
