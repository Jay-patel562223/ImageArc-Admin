export const API_ENDPOINTS = {
  ATTACHMENTS: 'attachments',
  ANALYTICS: '/admin/analytics',
  // ANALYTICS: 'analytics',
  ATTRIBUTES: 'attributes',
  ATTRIBUTE_VALUES: 'attribute-values',
  ORDER_STATUS: 'order-status',
  ORDERS: '/admin/orders',
  // ORDERS: 'orders',
  USERS: '/admin/users',
  // USERS: 'users',
  LOGIN: '/admin/login',
  REGISTER: '/admin/register',
  // REGISTER: 'register',
  PRODUCTS: '/admin/product',
  // PRODUCTS: 'products',
  POPULAR_PRODUCTS: 'popular-products',
  COUPONS: 'coupons',
  VERIFY_COUPONS: 'coupons/verify',
  CUSTOMERS: 'customers',
  TAXES: 'taxes',
  SHIPPINGS: 'shippings',
  // SETTINGS: '/admin/settings',
  SETTINGS: 'settings',
  
  CATEGORIES: '/admin/category',
  ACTIVECATEGORIES: '/user/category',
  PAGES: '/admin/page',
  PACKAGETYPE: '/admin/package_type',
  GetAllProductPrice: '/admin/getAllProductPrice',
  GetAllProductDpi: '/admin/getAllProductDpi',
  SUBSCRIPTION: '/admin/subscription_package',
  PRODUCT_PRICE: '/admin/product-prices',
  PRODUCT_DPI: '/admin/product-dpi',
  COUNTRIES:'/admin/countries',
  STATES:'/admin/states',
  STATES_COUNTRY:'/admin/state_country',
  CONTACTUS:'/admin/contactUs',
  // CATEGORIES: 'categories',
  TAGS: 'tags',
  TYPES: 'types',
  PROFILE_UPDATE: 'profile-update',
  LOGOUT: '/admin/logout',
  // LOGOUT: 'logout',
  ME: '/admin/getUserDetail',
  // ME: '/admin/loginUser',
  // ME: 'me',
  TOKEN: 'token',
  BLOCK_USER: '/admin/block-user',
  UNBLOCK_USER: '/admin/unblock-user',
  CHANGE_PASSWORD: '/admin/changePassword',
  // CHANGE_PASSWORD: 'change-password',
  FORGET_PASSWORD: '/user/forgetPassword',
  VERIFY_FORGET_PASSWORD_TOKEN: '/user/verifyPassword',
  // VERIFY_FORGET_PASSWORD_TOKEN: 'verify-forget-password-token',
  RESET_PASSWORD: '/user/resetPassword',
  // RESET_PASSWORD: 'reset-password',
  DOWNLOAD_INVOICE: 'download/invoice',
  APPROVE_SHOP: 'approve-shop',
  DISAPPROVE_SHOP: 'disapprove-shop',
  SHOPS: 'shops',
  MY_SHOPS: 'my-shops',
  WITHDRAWS: 'withdraws',
  APPROVE_WITHDRAW: 'approve-withdraw',
  ADD_WALLET_POINTS: 'add-points',
  REFUNDS: 'refunds',
  STAFFS: 'staffs',
  ADD_STAFF: 'staffs',
  REMOVE_STAFF: 'staffs',
  IMPORT_PRODUCTS: 'import-products/',
  IMPORT_ATTRIBUTES: 'import-attributes/',
  IMPORT_VARIATION_OPTIONS: 'import-variation-options/',
  MAKE_ADMIN: 'users/make-admin',
  AUTHORS: 'authors',
  MANUFACTURERS: 'manufacturers',
  CHECKOUT: 'orders/checkout/verify',
  QUESTIONS: 'questions',
  REVIEWS: 'reviews',
  ABUSIVE_REPORTS_DECLINE: 'abusive_reports/reject',
  ABUSIVE_REPORTS: 'abusive_reports',
  ORDER_EXPORT: 'export-order-url',
  ORDER_CREATE: 'order/create',
  ORDER_INVOICE_DOWNLOAD: '/admin/GenerateInvoice',
  TRANSACTION:"/admin/transactions",
  // ORDER_INVOICE_DOWNLOAD: 'download-invoice-url',
  SOCIAL_MEDIA:'/admin/social-media',
};
