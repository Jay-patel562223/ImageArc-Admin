...
import SystemConfigList from '@/components/systemConfig/systemConfig-list';
import { useSystemsConfigQuery } from '@/data/system-config';
...
export default function SystemConfig() {
  ...
  const { systemConfig, paginatorInfo, loading, error } = useSystemsConfigQuery({
    limit: 20,
    page,
    type,
    name: searchTerm,
        ...
        <div className="flex w-full flex-col items-center md:flex-row">
          <div className="mb-4 md:mb-0 md:w-1/4">
            <h1 className="text-xl font-semibold text-heading">
              {t('form:input-label-systemConfig')}
            </h1>
          </div>
          ...
          <div className="ms-auto flex w-full flex-col items-center space-y-4 md:flex-row md:space-y-0 xl:w-3/4">
            ...
            <div style={{flex: 'none',textTransform: 'uppercase'}}>
              {locale === Config.defaultLanguage && (
                <LinkButton
                  href={`${Routes?.systemConfig?.create}`}
                  className="md:ms-6 h-12 w-full md:w-auto"
                >
                  <span className="block md:hidden xl:block">
                    + {t('form:button-label-add-systemConfig')}
                  </span>
