import PageList from '@/components/static_pages/page-list';
...
import { usePagesQuery } from '@/data/pages';
...
export default function Pages() {
  ...
  const { pages, paginatorInfo, loading, error } = usePagesQuery({
    limit: 20,
    page,
    type,
    name: searchTerm,
          ...
          <div className="ms-auto flex w-full flex-col items-center space-y-4 md:flex-row md:space-y-0 xl:w-3/4" >
            ...
            <div style={{flex: 'none',textTransform: 'uppercase'}}>
              {locale === Config.defaultLanguage && (
                <LinkButton
                  href={`${Routes.pages.create}`}
                  className="md:ms-6 h-12 w-full md:w-auto"
                >
                  <span className="block md:hidden xl:block">
                    + {t('form:button-label-add-pages')}
                  </span>
